...
def check_point(P):
    if (P.x**2 - D*P.y**2 )% p == 1:
        return True
    return False


pka = Point(155781055760279718382374741001148850818103179141959728567110540865590463, 73794785561346677848810778233901832813072697504335306937799336126503714)
pkb = Point(171226959585314864221294077932510094779925634276949970785138593200069419, 54353971839516652938533335476115503436865545966356461292708042305317630)
...
ct = bytes.fromhex(enc_flag['encrypted_flag'])


# Using sage we mapped each point P (x,y) to a number u in F_p using u = x + y*sqrt(D) = x + 23y
# then we utilized discrete_log() to find k_a 
k_a = 85659350935432836744105386596354218808820976897571304400328185351959247

# and we calculated u_ab as b^k_a to find shared_integer
shared_integer = 83201481069630956436480435779471169630605662777874697301601848920266492


key = sha1(str(shared_integer).encode('ascii')).digest()[:16]
