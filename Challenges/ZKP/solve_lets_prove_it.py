...
MAX_USEFUL_PROOFS = 6  # see turn-economy note below
...
for c in top:
    t0 = time.time()
    extra = sympy.factorint(c["cofactor"], limit=10 ** 6)
    for prime, mult in extra.items():
        if prime.bit_length() < 90:  # keep only genuinely "small" factors we can BSGS
