...
# This file was *autogenerated* from the file solve_special_isogenies.sage
from sage.all_cmdline import *   # import sage library

_sage_const_419 = Integer(419); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3); _sage_const_420 = Integer(420); _sage_const_2 = Integer(2); _sage_const_5 = Integer(5); _sage_const_200 = Integer(200)
p = _sage_const_419 
F = GF(p)

E0 = EllipticCurve(F, [_sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_1 , _sage_const_0 ])          # y^2 = x^3 + x, i.e. M_0

assert p % _sage_const_4  == _sage_const_3 
assert E0.is_supersingular()
assert E0.order() == p + _sage_const_1  == _sage_const_420 
assert factor(p + _sage_const_1 ) == factor(_sage_const_420 )

# E(F_p) is cyclic -- this is the fact the whole challenge rests on
assert E0.abelian_group().invariants() == (_sage_const_420 ,), "E(F_p) is not cyclic!"
...
def montgomery_coefficient(E):
    ...
    out = []
    for A in F:
        if A**_sage_const_2  == _sage_const_4 :                # singular: x^3 + Ax^2 + x has a double root
            continue
        if EllipticCurve(F, [_sage_const_0 , A, _sage_const_0 , _sage_const_1 , _sage_const_0 ]).is_isomorphic(E):
            out.append(A)
    return out
...
cofactor = (p + _sage_const_1 ) // _sage_const_5 
codomains = []
generators = set()

for _ in range(_sage_const_200 ):
    P = cofactor * E0.random_point()
    if P.order() != _sage_const_5 :
        continue
    generators.add(frozenset(tuple(R.xy()) if R else None for R in [k * P for k in range(_sage_const_5 )]))
    phi = E0.isogeny(P)
    assert phi.degree() == _sage_const_5 
...
assert len(generators) == _sage_const_1 , "more than one order-5 subgroup -- group is not cyclic"

js = {E.j_invariant() for E in codomains}
assert len(js) == _sage_const_1 , "different points of order 5 gave different codomains"
print(f"distinct codomain j-invariants: {len(js)}  ->  j = {js.pop()}")

Ecod = codomains[_sage_const_0 ]
...
assert _sage_const_0  in montgomery_coefficient(E0)
