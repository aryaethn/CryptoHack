...
# This file was *autogenerated* from the file solve_abelian_sidh.sage
from sage.all_cmdline import *   # import sage library

_sage_const_66755491218549620204451278063200785887258235588279474221852899550437797658031 = Integer(66755491218549620204451278063200785887258235588279474221852899550437797658031); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_216 = Integer(216); _sage_const_137 = Integer(137); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_128 = Integer(128); _sage_const_16 = Integer(16)
import re
...
q = _sage_const_66755491218549620204451278063200785887258235588279474221852899550437797658031 
l_a, l_b = _sage_const_2 , _sage_const_3 
e_a, e_b = _sage_const_216 , _sage_const_137 
p = (l_a**e_a) * (l_b**e_b) * q - _sage_const_1 
assert is_pseudoprime(p) and is_pseudoprime(q)

F2 = GF(p**_sage_const_2 , name="i", modulus=[_sage_const_1 , _sage_const_0 , _sage_const_1 ])
i = F2.gen()

E = EllipticCurve(F2, [_sage_const_1 , _sage_const_0 ])
E.set_order(((l_a**e_a) * (l_b**e_b) * q)**_sage_const_2 )
...
def parse_point(name):
    line = re.search(rf"^{re.escape(name)} = \((.*)\)$", text, re.MULTILINE).group(_sage_const_1 )
    coords = TERM.findall(line)
    assert len(coords) == _sage_const_2 , f"expected two coordinates for {name}, got {len(coords)}"
    (ax, bx), (ay, by) = coords
...
iv_hex = re.search(r"iv = ([0-9a-f]+)", text).group(_sage_const_1 )
ct_hex = re.search(r"ct = ([0-9a-f]+)", text).group(_sage_const_1 )
...
assert set(cand_A) == set(cand_B),     "the two derivations disagree -- the public keys are not [l^e]G up to Aut"
...
for k, ss in enumerate(cand_A):
    key = SHA256.new(data=str(ss).encode()).digest()[:_sage_const_128 ]
